...
$address_rs = Database::search("SELECT `city_city_id` AS cid FROM `user_has_address` INNER JOIN `city` ON `user_has_address.city_city_id` = `city.city_id` INNER JOIN `district` ON `city.district_district_id`=district_district_id WHERE `user_email`='".$user."'");
...
$seller = $seller_data["fname"]." ".$seller_data[];
